...
let
  ...
  configSet = {
    ...
    // (if cfg.container-log-max-size != null { containerLogMaxSize = cfg.container-log-max-size; } else {})
    // (if cfg.container-log-max-files != null { containerLogMaxFiles = cfg.container-log-max-files; } else {})
    // (if cfg.config-map-and-secret-change-detection-strategy != null { configMapAndSecretChangeDetectionStrategy = cfg.config-map-and-secret-change-detection-strategy; } else {})
    // (if cfg.enforce-node-allocatable != null { enforceNodeAllocatable = cfg.enforce-node-allocatable; } else {})
    // (if cfg.volume-plugin-dir != null { volumePluginDir = cfg.volume-plugin-dir; } else {})
    // (if cfg.logging-format != null { logging = { format = cfg.logging-format; }; } else {})
    // (if cfg.logging-flush-frequency != null { logging = { flushFrequency = cfg.logging-flush-frequency; }; } else {})
    // (if cfg.v != null { logging = { verbosity = cfg.v; }; } else {})
    // (if cfg.log-json-info-buffer-size != null { logging = { options = { json = { infoBufferSize = cfg.log-json-info-buffer-size; }; }; }; })
    // (if cfg.enable-system-log-handler != null { enableSystemLogHandler = cfg.enable-system-log-handler; } else {})
    // (if cfg.enable-system-log-query != null { enableSystemLogQuery = cfg.enable-system-log-query; } else {})
    // (if cfg.shutdown-grace-period != null { shutdownGracePeriod = cfg.shutdown-grace-period; } else {})
    // (if cfg.shutdown-grace-period-critical-pods != null { shutdownGracePeriodCriticalPods = cfg.shutdown-grace-period-critical-pods; } else {})
    // (if cfg.enable-profiling-handler != null { enableProfilingHandler = cfg.enable-profiling-handler; } else {})
    // (if cfg.enable-debug-flags-handler != null { enableDebugFlagsHandler = cfg.enable-debug-flags-handler; } else {})
    // (if cfg.seccomp-default != null { seccompDefault = cfg.seccomp-default; } else {})
    // (if cfg.memory-throttling-factor != null { memoryThrottlingFactor = cfg.memory-throttling-factor; } else {})
    // (if cfg.register-node != null { registerNode = cfg.register-node; } else {})
    // (if cfg.local-storage-capacity-isolation != null { localStorageCapacityIsolation = cfg.local-storage-capacity-isolation; } else {})
    // (if cfg.container-runtime-endpoint != null { containerRuntimeEnpoint = cfg.container-runtime-endpoint; } else {})
    ;
