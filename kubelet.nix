...
let
  ...
  kubeConfigSet = {
    ...
    users = [{
      name = "kubelet";
      user = {
        client-certificate = cfg.kubeConfigOpts.certCrtFile;
        client-key = cfg.kubeConfigOpts.tcertKeyFile;
      };
    }];
    ...
    ;
  mkKubeConfig = name: pkgs.writeText "${name}-kubeconfig" (builtins.toJSON kubeConfigSet);
