...
let
  ...
  generateCaCert = pkgs.writeScriptBin "generate-ca-cert" ''
    ...
    ${pkgs.openssl}/bin/openssl req -x509 -newkey rsa:4096 -pass pass:"$CA_PASSWORD" -keyout "$SSL_FOLDER/$CA_NAME.key" -out "$SSL_FOLDER/$CA_NAME.pem" -days $DAYS -nodes -subj "/C=$C/ST=$ST/L=$L/O=$O/OU=$OU/CN=$CN"
  '';
in
{
